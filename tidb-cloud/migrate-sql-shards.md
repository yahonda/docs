...
3.  **Amazon S3からデータをインポートする**ページで、以下の情報を入力してください。
    ...
    -   **インポートするファイル数**： TiDB Cloud StarterまたはTiDB Cloud Essentialの場合は、 **Multiple files**を選択してください。このフィールドはTiDB Cloud Dedicatedでは利用できません。
    ...
    -   **Bucket Access**&gt; **AWSロールARN** ：取得したロールARNを入力してください。
