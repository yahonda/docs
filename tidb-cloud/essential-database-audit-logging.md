...
> **Note:**
>
> トラフィックの可視性を向上させるため、 `CLIENT_IP`では、ロードバランサー (LB) IP の代わりに、AWS PrivateLink 経由の接続の実際のクライアント IP アドレスが表示されるようになりました。現在、この機能はパブリックプレビューであり、AWS リージョン`Frankfurt (eu-central-1)`でのみ利用可能です。
