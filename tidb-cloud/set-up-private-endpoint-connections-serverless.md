...
4.  **Service Name**、**アベイラビリティゾーンID** 、**Region ID**をメモしておいてください。
...
1.  **VPC ID**と**Subnet ID**を取得するには、AWSマネジメントコンソールに移動し、該当するセクションでそれらを見つけてください。[ステップ1](#step-1-choose-a-tidb-instance)で見つけた**アベイラビリティゾーンID**を必ず入力してください。
...
> **Note:**
>
> 現在、エンドポイント占有モデルは、一部のAWSリージョンにおいて、2026年7月1日以降に作成された {{{ .essential }}} インスタンスでのみ利用できます。お使いのインスタンスで利用できない場合は、代わりに [エンドポイント共有モデル](#set-up-a-private-endpoint-with-aws-endpoint-shared-model) を使用できます。
