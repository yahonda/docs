...
## ステップ4: {{{ .starter }}} インスタンスに接続する {#step-4-connect-to-your-starter-instance}

TiDB は MySQL プロトコルとの高い互換性があるため、ほとんどの MySQL ツール、ドライバー、ORM を使用して接続できます。以下の手順では、MySQL CLI クライアント `mysql` を例として使用します。

1. [**My TiDB**](https://tidbcloud.com/tidbs) ページに移動し、対象の {{{ .starter }}} インスタンスの名前をクリックして概要ページに移動します。
...
MySQL CLI クライアントが開き、{{{ .starter }}} インスタンスに接続されます。
