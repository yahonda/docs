...
3.  操作を確認してください。バックアップはTiDB Cloudに保存され、**Backup List**に表示されます。
...
-   **Snapshot Restore**：特定のバックアップスナップショットからインスタンスを復元します。この方法は、自動バックアップと手動バックアップの両方の復元に使用できます。**Backup List**では、手動バックアップには**「手動」**タイプと**「永続的」有効**期限ステータスが表示されます。
