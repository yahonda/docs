...
-   SQL実行エンジン
    ...
    -   `JSON_UNQUOTE`関数がJSONテキストを処理するときに誤った結果が返される問題を修正 [＃11955](https://github.com/pingcap/tidb/pull/11955)
    ...
    -   `PREPARE`文の`GROUPBY`解析ルールが間違っている問題を修正 [＃12351](https://github.com/pingcap/tidb/pull/12351)
    -   ポイントクエリで権限チェックが正しく行われない問題を修正 [＃12340](https://github.com/pingcap/tidb/pull/12340)
    -   監視レコードに、ステートメント`PREPARE`の`sql_type`による期間が表示されない問題を修正しました。 [＃12331](https://github.com/pingcap/tidb/pull/12331)
    -   ポイントクエリ内のテーブルの別名の使用をサポートします（例： `select * from t tmp where a = "aa"` ） [＃12282](https://github.com/pingcap/tidb/pull/12282)
    -   BIT 型の列に負の数を挿入するときに負の値を符号なしとして処理しない場合に発生するエラーを修正しました。 [＃12423](https://github.com/pingcap/tidb/pull/12423)
...
-   サーバ
    ...
    -   TiDB スロークエリログの`Index_ids`フィールドを`Index_names`に置き換えて、スロークエリログの使いやすさを向上させます。 [＃12061](https://github.com/pingcap/tidb/pull/12061)
...
-   サーバ
    ...
    -   gRPC コアダンプの問題を修正し、OOM 回避するために共有メモリをサポートします。 [＃5524](https://github.com/tikv/tikv/pull/5524)
...
-   隣接する小さいリージョンを結合できない問題を修正[＃1726](https://github.com/pingcap/pd/pull/1726)
...
-   リージョンシンカーのTLSをサポート [＃1739](https://github.com/pingcap/pd/pull/1739)
...
-   TiDB Lightning
    ...
    -   `AUTO_INCREMENT`のアルゴリズムを最適化して、 `AUTO_INCREMENT`列のオーバーフローのリスクを軽減します。 [＃227](https://github.com/pingcap/tidb-lightning/pull/227)
